            ...
            if batch_i % img_step == 0:
                ...
                recon = sess.run(
                    ae['x_tilde'], feed_dict={
                        ae['z']: zs})
                print('Manifold: recon:', recon.min(), recon.max())
                ...
                recon = sess.run(
                    ae['x_tilde'], feed_dict={
                        ae['x']: test_xs})
                print('Recons: recon:', recon.min(), recon.max())
                recon = np.clip(recon / recon.max(), 0, 1)
                if (on_cloud==0):
                    utils.montage(recon.reshape([-1] + crop_shape),
...
def test_celeb(n_epochs=100, crop_shape=[100, 100, 3],
    ...
    if (on_cloud==0):
        ...
        if not os.path.exists('./output/reconstruct'):
            os.makedirs('./output/reconstruct')
        if not os.path.exists('./output/manifolds'):
            os.makedirs('./output/manifolds')        
    else:
