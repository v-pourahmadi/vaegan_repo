...
def conv2d_gen(x, n_output,
           k_h=5, k_w=5, d_h=2, d_w=2,
           padding='SAME', name='conv2d_gen', reuse=None):
    """Helper for creating a 2d convolution operation.
    ...
    op : tf.Tensor
        Output of convolution
    """
    with tf.variable_scope(name or 'conv2d_gen', reuse=reuse):
        W_gen = tf.get_variable(
            name='W_gen',
            shape=[k_h, k_w, x.get_shape()[-1], n_output],
            initializer=tf.contrib.layers.xavier_initializer_conv2d())

        conv_gen = tf.nn.conv2d(
            name='conv_gen',
            input=x,
            filter=W_gen,
            strides=[1, d_h, d_w, 1],
        ...
        b_gen = tf.get_variable(
            name='b_gen',
            shape=[n_output],
            initializer=tf.constant_initializer(0.0))

        h_gen = tf.nn.bias_add(
            name='h_gen',
            value=conv_gen,
            bias=b_gen)
